...
can do X but we don't wrap it yet". Tracked changes is a partial example: the read side is wrapped
(`Document.trackedChanges()` → `TrackedChanges`, see N12), while accept/reject, authoring, and
advanced revision types still fall back to `raw()` until their sub-tasks land.
...
**范围(只读)**: 本子任务只覆盖**只读消费**——`enabled()`(开关状态)、`list()`(按文档顺序枚举)、`get(id)`(按稳定 id 命中,miss 抛 `NoSuchElementException`)。**不**含:开关写入、accept/reject、显式创作(authoring)、高级修订类型。这些仍走 `raw()` 直到各自子任务落地。回归:`TrackedChangesTest`(10 用例);全量 138 tests green、spotless clean;手工往返验证带 `<w:ins>` 的 docx save→reopen 修订正确重现。
