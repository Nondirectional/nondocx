...
text-class accept/reject, and text-class authoring are wrapped (`Document.trackedChanges()` /
`Paragraph` / `Run`, see N12 / N13 / N14), while move/property/cell revision types (and accept/
reject/authoring of those types) still fall back to `raw()` until the `advanced-types` sub-task
lands.
...
**范围(文本类创作)**: 本子任务覆盖 `ins`/`del` 的显式创作(insertion/deletion/replacement)。**不**含:`move`/属性类/`cellIns`/`cellDel` 的创作、开关写入、"自动追踪所有既有写操作"。回归:`TrackedAuthoringTest` 10 用例(读回 / 元数据 / 正交 / 普通 API 无污染 / 与 accept 集成 / 边界);全量 159 tests green、spotless clean。
