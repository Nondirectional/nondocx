...
can do X but we don't wrap it yet". Tracked changes is a partial example: the read side and
text-class accept/reject are wrapped (`Document.trackedChanges()` → `TrackedChanges`, see N12 /
N13), while authoring, move/property/cell revision types, and accept/reject of those types still
fall back to `raw()` until their sub-tasks land.
...
**范围(文本类写)**: 本子任务覆盖 `ins`/`del` 的 accept/reject(all/byAuthor/单条 id 三粒度)。**不**含:`move`/属性类/`cellIns`/`cellDel` 的写语义、开关写入、显式创作(authoring)。回归:`TrackedChangesTest` 新增 11 用例(accept/reject × ins/del × all/byAuthor/id + 表格内 + 边界);全量 149 tests green、spotless clean。
