...
入口只有一个工厂类 [`Docx`](../nondocx-core/src/main/java/com/non/docx/core/Docx.java)。`Docx.open(...)` 返回一个 `Document` —— 它是 `AutoCloseable`，用 try-with-resources 包住。
...
> 取自 [`FormattingDemo.java`](../nondocx-examples/src/main/java/com/non/docx/examples/FormattingDemo.java)，可直接运行。
...
> 取自 [`HeaderFooterExample.java`](../nondocx-examples/src/main/java/com/non/docx/examples/HeaderFooterExample.java)。
