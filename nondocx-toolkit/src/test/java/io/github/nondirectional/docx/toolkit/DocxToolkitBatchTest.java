...
class DocxToolkitBatchTest {
  ...
  void shouldRejectMalformedEditEntry(@TempDir Path tmp) throws Exception {
    ...
    assertThat(io.github.nondirectional.docx.toolkit.ToolTestSupport.parse(result).code())
        .isEqualTo(io.github.nondirectional.docx.toolkit.result.ToolResultCode.PARTIAL_FAILURE);
  ...
  private static String firstHyperlinkUrl(Document doc) {
    io.github.nondirectional.docx.core.api.text.Hyperlink link = firstHyperlink(doc);
  ...
  private static io.github.nondirectional.docx.core.api.text.Hyperlink firstHyperlink(
      Document doc) {
    return doc.paragraphs().get(0).inlineElements().stream()
        .filter(e -> e instanceof io.github.nondirectional.docx.core.api.text.Hyperlink)
        .map(e -> (io.github.nondirectional.docx.core.api.text.Hyperlink) e)
        .findFirst()
        .orElse(null);
  }
