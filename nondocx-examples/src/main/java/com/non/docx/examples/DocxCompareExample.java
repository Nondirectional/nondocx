 ...
 *
 * <p><b>第二层：POI 如何表达。</b> Apache POI 负责把 docx 打开成 {@code XWPFDocument} / {@code XWPFParagraph} / {@code
 * XWPFRun} 这样的对象，但它没有“比较两份 docx 并产出修订”的高层 API。
 *
 * <p><b>第三层：nondocx 为什么这样设计。</b> nondocx 把 compare 入口放在 {@link Docx}：{@link Docx#compare(Path, Path)}
 * / {@link Docx#compare(Path, Path, String)}。第一版以<b>旧文档</b>为基线，只比较<b>正文纯文本段落</b>，把差异重放成标准修订；
 * 表格等非支持结构保留旧文档原样。
 */
public final class DocxCompareExample {
  ...
  private static void createNewVersion(Path file) throws Exception {
    try (Document doc = Docx.create()) {
      doc.addParagraph("版本对比演示");
      doc.addParagraph("本周完成接口联调和回归测试。");
      doc.addParagraph("新增安排：下周开始灰度发布。");
  ...
  private static void inspectComparedFile(Path file, String label) throws Exception {
    try (Document doc = Docx.open(file)) {
      ...
      System.out.println("[" + label + "] 表格首单元格仍是旧值: " + doc.tables().get(0).row(0).cell(0).text());
    }
  }
  ...
  private static void inspectDefaultAuthorResult(Path file) throws Exception {
    try (Document doc = Docx.open(file)) {
      ...
      System.out.println("[默认作者结果] DEFAULT_COMPARE_AUTHOR = \"" + Docx.DEFAULT_COMPARE_AUTHOR + "\"");
      for (int i = 0; i < changes.size(); i++) {
