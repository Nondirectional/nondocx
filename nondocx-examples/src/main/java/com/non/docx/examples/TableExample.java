 ...
 *
 * <p>OOXML 中表格的结构是 {@code <w:tbl>} → {@code <w:tr>}（行）→ {@code <w:tc>}（单元格）。
 * 每个 {@code <w:tc>} 内部又包含段落，所以单元格可以像文档正文一样有格式化文本。</p>
 *
 * <p>nondocx 的 {@code Table.row(Consumer<Row>)} 便捷方法让你能用 lambda 快速填充行。
 * 行内的 {@code row.cell("文本")} 追加一个单文本单元格；{@code row.cell(c -> c.text("..."))} 或
 * {@code c.addParagraph().addRun(...)} 用于复杂内容。</p>
 */
