 ...
 *
 * <p>页眉 <em>不是</em> 正文元素 — 它位于文档正文之外，通过页眉引用附加到 {@link
 ...
 * 实例但具有相同段落的页眉是相等的 — 这就是 往返断言能正常工作的原因。
 *
...
public final class Header {
   ...
   * 封装给定的 POI 页眉。
   *
   * <p>此构造函数是 {@link io.github.nondirectional.docx.core.api.section.Section} 生成活跃页眉包装器的内部接缝，
   * 因此它有意接受 POI 类型。用户通过 {@code Section.header()} / {@code Document.header()} 获取页眉，而不是直接构造它们。
   *
