...
import io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes;
 ...
 *
 * <p>由 {@link io.github.nondirectional.docx.core.api.Document#trackedChanges()} 返回。本门面负责两类事:
 *
 * <p><b>只读消费</b>:
 *
 * <ul>
 ...
 * io.github.nondirectional.docx.core.api.text.Run#addInsertion} 等)直接产出修订标记,不依赖开关状态。
 *
...
public final class TrackedChanges {
   ...
   * 封装给定的 POI 文档以解析其修订。
   *
   * <p>此构造函数是 {@link io.github.nondirectional.docx.core.api.Document} 生成修订视图的<b>内部接缝</b>,因此它有意接受
   * POI 类型 (与 {@code TableOfContents(XWPFDocument)} 接受 {@code XWPFDocument} 的方式一致,见 poi-bridge.md
   * N1)。用户通过 {@code Document.trackedChanges()} 获取,而不是直接构造。
   *
