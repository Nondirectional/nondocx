 ...
 * {@code io.github.nondirectional.docx.core.api.text.Run}）的 形式呈现。同时携带 <em>文本和</em> 图片的运行是 MVP
 * 未完全建模的 边界情况 — 在这种情况下文本部分不会单独呈现， 只能通过 {@code raw()} 访问。大多数文档以及 {@code Paragraph.addImage(...)}
...
public final class Image implements InlineElement {
   ...
   * 封装给定的 POI 内联图片。
   *
   * <p>此构造函数是 {@link io.github.nondirectional.docx.core.api.text.Paragraph} 生成活跃图像包装器的内部接缝， 因此它有意接受
   * POI 类型。用户通常通过 {@code Paragraph.addImage(...)} 获取图像， 而不是直接构造它们。
   *
