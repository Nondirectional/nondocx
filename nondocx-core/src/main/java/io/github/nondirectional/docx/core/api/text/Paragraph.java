...
import io.github.nondirectional.docx.core.api.exception.DocxOperationException;
...
import io.github.nondirectional.docx.core.internal.poi.Numbering;
import io.github.nondirectional.docx.core.internal.poi.Pictures;
...
public final class Paragraph implements BodyElement {
   ...
   *     null}
   * @see io.github.nondirectional.docx.core.api.track.TrackedChanges
   */
  public Run addInsertion(String author, String text) {
    ...
    java.math.BigInteger revisionId =
        io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes.nextRevisionId(doc);
    org.openxmlformats.schemas.wordprocessingml.x2006.main.CTRunTrackChange ins =
        io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes.addInsertion(
   ...
   * POI/XmlBeans 脏活下沉到 {@code internal/poi/CommentNodes}。范围语义为<b>整段</b>:批注的 {@code
   * commentRangeStart} 在段首、{@code commentRangeEnd} 在段末,包住本段全部已有内容。
   *
   * <p><b>OOXML / POI / nondocx 三层。</b>
   *
   * <ul>
   *   <li><b>OOXML</b>:批注在 OOXML 里是「正文锚点 + 批注正文」分离的两 part 结构。本段正文里插 {@code <w:commentRangeStart
   *       w:id=../>}(段首)与 {@code <w:commentRangeEnd w:id=../>}(段末) + 一个 {@code <w:commentReference
   *       w:id=../>} 引用 run(段末);{@code word/comments.xml} 里加 {@code <w:comment w:id=.. w:author=..
   *       w:date=..>} 含批注正文。四处用同一 {@code w:id} 配对。
   ...
   *       排序(会落到段末、范围为空),必须 XmlCursor 手动 move 到段首——这是批注创作区别于 tracked insertion (新建容器包新
   *       run、顺序天然正确)的核心脏活。
   *   <li><b>nondocx</b>:把「建 comments.xml 条目 + 正文三锚点 + XmlCursor 定位」全收进 {@code
   *       internal/poi/CommentNodes.addWholeParagraphComment},对外只暴露本方法,返回 POI-free 的 {@link
   *       Comment}。
   * </ul>
   *
   * <p><b>返回值。</b> 返回新建的 {@link Comment}(holding-wrapper,持底层 {@code XWPFComment} 委托)。调用方可立即 读
   * {@link Comment#id()} / {@link Comment#author()} / {@link Comment#text()} 拿到刚写入的元数据,无需二次查询;
   ...
   *
   * <p><b>initials。</b> 当前设为空串(不派生)——OOXML 不约束 initials 与 author 的关系,空 initials 不影响 Word 显示;若 AC4
   * 人工验收发现 Word 显示需要 initials 再回退补派生逻辑。
   *
   ...
   * @throws IllegalArgumentException 如果 {@code author} 为 {@code null} 或空白,或 {@code text} 为 {@code
   *     null}
   * @see io.github.nondirectional.docx.core.api.comment.Comments
   * @see io.github.nondirectional.docx.core.api.comment.Comment
   */
  public Comment addComment(String author, String text) {
    ...
    java.math.BigInteger commentId =
        io.github.nondirectional.docx.core.internal.poi.CommentNodes.nextCommentId(doc);
    org.apache.poi.xwpf.usermodel.XWPFComment created =
        io.github.nondirectional.docx.core.internal.poi.CommentNodes.addWholeParagraphComment(
  ...
  public Paragraph addDeletion(String author, Run target) {
    ...
    java.math.BigInteger revisionId =
        io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes.nextRevisionId(doc);
    io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes.addDeletion(
  ...
  public List<Run> moveRunsFrom(String author, Paragraph sourceParagraph, List<Run> runs) {
    ...
    List<org.openxmlformats.schemas.wordprocessingml.x2006.main.CTR> movedCtrs =
        io.github.nondirectional.docx.core.internal.poi.TrackedChangeNodes.moveRuns(
