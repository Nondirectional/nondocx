...
/**
 * A hyperlink — an inline fragment whose visible text links to a target (typically an external URL).
 *
 ...
 * the owning document to the external target. The document is reached via the run's own
 * {@code getDocument()} reference, so no extra parameter is needed; if the relationship cannot be
 * resolved (for example the run is detached from its document, or the hyperlink targets an internal
 * anchor rather than a URL), {@code url()} returns {@code null}.
 *
...
public final class Hyperlink implements InlineElement {
     ...
     * wrappers, so it accepts a POI type by design. Users normally obtain hyperlinks via
     * {@code Paragraph.addHyperlink(...)} rather than constructing them.
     *
     ...
     *
     * @return the backing {@code XWPFHyperlinkRun} instance (same instance for the wrapper's lifetime)
     */
