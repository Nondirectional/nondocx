 ...
 * landscape orientation the stored {@code <w:pgSz>} width/height are swapped by {@link
 * Section#orientation(Orientation)}; {@link #fromDimensions(int, int)} normalizes back to portrait
 * so the logical paper size can be recovered regardless of orientation.
 *
...
public enum PaperSize {
   ...
   *
   * <p>The comparison is orientation-agnostic: the inputs are normalized to portrait (smaller
   * dimension first) before matching, so the same logical paper size is resolved whether the
   * section is portrait or landscape. Matching is exact; custom or uncommon dimensions return
   * {@code null}.
   *
