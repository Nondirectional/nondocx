 ...
 * nondocx's mutable live objects. Every method delegates to the live {@link Document} /
 * {@link Paragraph} / {@link Table} building blocks ({@code addParagraph}, {@code addTable},
 * {@code addRun}, ...). No run, paragraph, or table behavior is duplicated here — the builder only
...
public final class DocumentBuilder {
    ...
    /**
     * Appends a new, empty paragraph, applies the given configurator to it, and returns this
     * builder.
     *
     * <p>The configurator operates on the live {@link Paragraph}, so it has the full run and
     * paragraph-style API available — for example
     * {@code .paragraph(p -> p.addRun("hi").bold().fontSize(14))} or
     * {@code .paragraph(p -> p.heading(HeadingLevel.H2).addRun("section"))}. No run or style logic
     * is duplicated here; every call reaches the live {@code Paragraph}.
     *
     ...
     * {@code .table(t -> t.row(r -> r.cell("A1").cell("B1")).row(r -> r.cell("A2").cell("B2")))}.
     * No row or cell logic is duplicated here; every call reaches the live {@code Table}.
     *
     ...
     * chain. It is the same kind of {@link Document} obtained from {@link Docx#create()} or
     * {@link Docx#open}, so it composes with the rest of the API (save it, keep mutating it, etc.).
     * The caller owns it and should close it when done.
     *
