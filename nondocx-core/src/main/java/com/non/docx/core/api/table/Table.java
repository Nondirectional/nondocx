 ...
 * sequence of rows from top to bottom, each a live view over its cells. Content equality ({@code
 * equals} / {@code hashCode}) compares that ordered row sequence, never the delegate reference, so
 * two tables over distinct POI instances but with the same rows are equal — this is what makes
...
public final class Table implements BodyElement {
   ...
   * wrappers, so it accepts a POI type by design. Users normally obtain tables via {@code
   * Document.tables()} / {@code Document.addTable()} rather than constructing them directly.
   *
  ...
  public void removeRow(int index) {
    ...
    if (index < 0 || index >= size) {
      throw new IndexOutOfBoundsException(
          "row index " + index + " out of bounds (table has " + size + " rows)");
    }
