...
public final class Comments {
   ...
   * 对一条已有批注回复,返回新建的回复批注。
   *
   * <p>回复 = 普通批注 + 线程关系。创作入口在<b>门面</b>(本方法),不在内容类型——对照 {@code tc.accept}(处理已有结构的
   * 破坏性写在门面)。与 {@code Paragraph.addComment}(创作新根批注)分工:addComment 建无父的根批注,reply 建挂父的子批注。
   *
   * <p><b>OOXML / POI / nondocx 三层。</b>
   *
   * <ul>
   *   <li><b>OOXML</b>:回复在 comments.xml 里就是新的 {@code <w:comment>}(同普通批注);线程关系靠
   *       {@code commentsExtended.xml} 的 {@code <w15:commentEx w15:paraId=.. w15:paraIdParent=父paraId/>};正文锚点紧贴父批注
   *       锚点。durableId/dateUtc 协作元数据同步写入 commentsIds/commentsExtensible。
   *   <li><b>POI</b>:comments.xml 用 {@code createComment}(复用);三个扩展 part POI <b>无 API</b>,nondocx 用 OPC 层自维护
   *       (探针见 {@code research/part-lifecycle.md})。
   *   <li><b>nondocx</b>:comments.xml 条目 + 正文锚点 + 四 part 追加全收进 {@code internal/poi/CommentNodes.replyToComment}
   *       与 {@code internal/poi/CommentExtendedParts},对外只暴露本方法,返回 POI-free 的 {@link Comment}。
   * </ul>
   *
   * <p><b>返回值。</b> 返回新建的回复批注,{@link Comment#parentId()} 命中 {@code parentId} 参数。author 必传(与
   * {@code addComment} 对称);date/{@code w:id}/paraId/durableId 自动分配。
   *
   ...
   * @return 新建的回复批注
   * @throws IllegalArgumentException 如果 {@code parentId} 为 {@code null},{@code author} 为 {@code null} 或空白, 或 {@code text} 为 {@code null}
