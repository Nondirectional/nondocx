 ...
 *
 * <p>Bridge between nondocx's POI-free {@link ListKind} and Apache POI's OOXML numbering
 * machinery. This is the only place that builds {@code CTAbstractNum}/{@code CTNum} definitions
 * and reads list membership back off an {@code XWPFParagraph}, so the public value object
 * {@code ListKind} and the {@code Paragraph} wrapper stay free of {@code org.apache.poi.*} at the
 * source level.
 *
 ...
 * document; all paragraphs of the same kind share that num and differ only by their
 * {@code ilvl}. The {@code numId} for each (document, kind) is cached so repeated
 * {@code list(...)} calls never create duplicate definitions on the same document.
 *
 ...
 * lifetime; this is acceptable for an MVP library, but means documents should not be expected to
 * be garbage-collected while the cache retains them. After a {@code save} → {@code open} round
 * trip the freshly opened document is a different {@code XWPFDocument} instance, so its list
 * definitions are re-created on demand the first time {@code list(...)} is applied again; this is
 * harmless because paragraphs keep resolving correctly via the numId stored in the file.
 */
public final class Numbering {
    ...
    /**
     * Per-document cache of the {@code numId} provisioned for each {@link ListKind}. Keyed by
     * {@code XWPFDocument} identity so the same document does not get duplicate numbering
     * definitions across repeated {@code list(...)} calls.
     */
    private static final Map<XWPFDocument, Map<ListKind, BigInteger>> NUM_ID_CACHE = new IdentityHashMap<>();

    private Numbering() {
    }

    /**
     * Marks the given paragraph as a member of a list of the specified kind at the specified
     * nesting level. Ensures the owning document has a numbering part and provisions a shared
     * abstractNum/num for the kind (cached), then assigns the paragraph's {@code numId} and
     * {@code ilvl}.
     *
     ...
     * save/open round trip. After this call {@link XWPFParagraph#getNumID()} reports {@code null},
     * so the paragraph reads as a non-list paragraph.
     *
     ...
     *
     * <p>The kind is inferred from the paragraph's current-level number format as reported by POI:
     * a {@code "bullet"} format means {@link ListKind#BULLET}; any other format (decimal, letter,
     * roman, …) collapses to {@link ListKind#NUMBERED}. A paragraph with no {@code numId} is
    ...
    /**
     * Returns the {@code numId} for the given (document, kind), provisioning a new
     * abstractNum+num on first access and serving it from the cache afterwards.
     */
    private static BigInteger numIdFor(XWPFDocument document, XWPFNumbering numbering, ListKind kind) {
