 ...
 * problem).
 *
 * <p>Carries an optional context string (e.g. a paragraph index or body position) via
 * {@link #getContext()} to aid diagnosis.
 *
 * <p><b>Guidance for callers and implementors:</b> argument-validation failures and out-of-bounds
 * index access reuse the standard JDK exceptions — {@link IllegalArgumentException} and
 * {@link IndexOutOfBoundsException} respectively — rather than this type. Reserve
 * {@code DocxOperationException} for genuine domain operation failures (for example: performing an
 * operation on an element whose state cannot satisfy it).
 *
 * <p>Example message: {@code "Cannot remove the only paragraph of a table cell (context: cell[0])"}.
 */
